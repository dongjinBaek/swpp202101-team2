...
const static string tReg = "(r([1-9]|1[0-9]|2[0-9]|3[0-2])|arg([1-9]|1[0-6])|sp)";
const static string tOptReg = PAREN(tReg + "|" + "_");
const static string tConst = "([0-9]+)";
const static string tValue = PAREN(tReg + "|" + tConst);
const static string tOptValue = PAREN(tValue + "|" + "_");
...
const static string tVLoad = "(vload)";
const static string tVStore = "(vstore)";
const static string tCool = "(cool)";
...
const static string rOptRegs2 = PAREN(tOptReg + "((" + tSpace + tOptReg + "){1})");
const static string rOptRegs4 = PAREN(tReg + "((" + tSpace + tOptReg + "){3})");
const static string rOptRegs8 = PAREN(tReg + "((" + tSpace + tOptReg + "){7})");
const static string rOptValues2 = PAREN(tOptValue + "((" + tSpace + tOptValue + "){1})");
const static string rOptValues4 = PAREN(tOptValue + "((" + tSpace + tOptValue + "){3})");
const static string rOptValues8 = PAREN(tOptValue + "((" + tSpace + tOptValue + "){7})");
...
const static string rVLoad2 = SPACED(rOptRegs2 + tAssign + tVLoad + tSpace + "2" + tSpace + tValue + tSpace + tConst);
const static string rVLoad4 = SPACED(rOptRegs4 + tAssign + tVLoad + tSpace + "4" + tSpace + tValue + tSpace + tConst);
const static string rVLoad8 = SPACED(rOptRegs8 + tAssign + tVLoad + tSpace + "8" + tSpace + tValue + tSpace + tConst);
const static string rVStore2 = SPACED(tVStore + tSpace + "2" + tSpace + rOptValues2 + tSpace + tValue + tSpace + tConst);
const static string rVStore4 = SPACED(tVStore + tSpace + "4" + tSpace + rOptValues4 + tSpace + tValue + tSpace + tConst);
const static string rVStore8 = SPACED(tVStore + tSpace + "8" + tSpace + rOptValues8 + tSpace + tValue + tSpace + tConst);
const static string rCool = SPACED(tCool + tSpace + tValue);
...
Reg parse_opt_reg(const string& optreg) {
  if (optreg == "_")
    return RegNone;
  else
    return parse_reg(optreg);
}
...
Value parse_opt_value(const string& optval) {
  if (optval == "_")
    return Value(RegNone);
  else
    return parse_value(optval);
}
...
Stmt* parse_vload(int line, const string& instr, VSize size) {
  int n = vsize_of(size);
  string rep = regex_replace(instr, regex(tAssign), " ");
  stringstream ss;
  string token;
  ...
  return new StmtVLoad(line, size, regs, ptr, ofs);
}

Stmt* parse_vstore(int line, const string& instr, VSize size) {
  int n = vsize_of(size);
  string rep = regex_replace(instr, regex(tAssign), " ");
  stringstream ss;
  string token;
  ...
  for (int i = 0; i < n; i++) {
    ss >> token;
    values.emplace_back(parse_opt_value(token));
  }
  ...
  return new StmtVStore(line, size, values, ptr, ofs);
}

Stmt* parse_cool(int line, const string& instr) {
  string rep = regex_replace(instr, regex(tAssign), " ");
  stringstream ss;
  string token;
  ss << rep;
...
Stmt* parse_normal_stmt(int line, const string& instr) {
  ...
  if (regex_match(instr, regex(rVLoad2)))
    return parse_vload(line, instr, VSize2);
  if (regex_match(instr, regex(rVLoad4)))
    return parse_vload(line, instr, VSize4);
  if (regex_match(instr, regex(rVLoad8)))
    return parse_vload(line, instr, VSize8);
  if (regex_match(instr, regex(rVStore2)))
    return parse_vstore(line, instr, VSize2);
  if (regex_match(instr, regex(rVStore4)))
    return parse_vstore(line, instr, VSize4);
  if (regex_match(instr, regex(rVStore8)))
    return parse_vstore(line, instr, VSize8);
  if (regex_match(instr, regex(rCool)))
    return parse_cool(line, instr);
