...
public:
  StmtMalloc(int _line, Reg _lhs, const Value& _val);
...
public:
  explicit StmtFree(int _line, const Value& _ptr);
...
public:
  StmtLoad(int _line, Reg _lhs, MSize _size, const Value& _ptr, uint64_t _ofs);
...
public:
  StmtStore(int _line, MSize _size, const Value& _val, const Value& _ptr, uint64_t _ofs);

  double exec(RegFile& regfile, Memory& memory) const override;
};

class StmtVLoad: public Stmt {
private:
  const VSize size;
  bool mask[8];
  Reg lregs[8];
...
public:
  StmtVLoad(int _line, VSize _size, const Reg* _lregs, const Value& _ptr, uint64_t _ofs);

  double exec(RegFile& regfile, Memory& memory) const override;
};

class StmtVStore: public Stmt {
private:
  const VSize size;
  bool mask[8];
  vector<Value> values;
...
public:
  StmtVStore(int _line, VSize _size, vector<Value> &_values, const Value& _ptr, uint64_t _ofs);

  double exec(RegFile& regfile, Memory& memory) const override;
};

class StmtVCool: public Stmt {
private:
  const Value ptr;

public:
  StmtVCool(int _line, const Value& _ptr);
...
public:
  ...
  string_view get_bb(const RegFile& regfile);
  bool get_eval() const;
  double exec(RegFile& regfile, Memory& memory) const override;
};
...
public:
  ...
  string_view get_bb(const RegFile& regfile) const;
  double exec(RegFile& regfile, Memory& memory) const override;
};
